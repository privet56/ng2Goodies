...
COLORREF GetLogLineColor(LPCTSTR pszMessage, UINT eMsgType)
{
	CString sMsg(pszMessage);
	sMsg.MakeLower();
	if(sMsg.Find("err:") > -1)		return RGB(255, 0, 0);
	if(sMsg.Find("error:") > -1)	return RGB(255, 0, 0);

	if(sMsg.Find("wrn:") > -1)		return RGB(128, 0, 128);
	if(sMsg.Find("warn:") > -1)		return RGB(128, 0, 128);
	if(sMsg.Find("warnung:") > -1)	return RGB(128, 0, 128);
	if(sMsg.Find("warning:") > -1)	return RGB(128, 0, 128);

	if(sMsg.Find("inf:") > -1)		return RGB(0, 0, 215);
	if(sMsg.Find("info:") > -1)		return RGB(0, 0, 215);
		...
		for (int i = 0; i < m_astrBuff.GetSize(); i++)
		{
			...
			if (!rstrLine.IsEmpty())
			{
				if ((_TUCHAR)rstrLine[0] < 8)
					AddLine((LPCTSTR)rstrLine + 1, rstrLine.GetLength() - 1, false, GetLogLineColor((LPCTSTR)rstrLine,(_TUCHAR)rstrLine[0]));
				else
...
void CHTRichEditCtrl::AddTyped(LPCTSTR pszMsg, int iLen, UINT eMsgType)
{
	...
	{
		FlushBuffer();
		AddLine(pszMsg, iLen, false, GetLogLineColor(pszMsg, eMsgType & LOGMSGTYPEMASK));
	}
}
int CHTRichEditCtrl::CharFromPos(CPoint pt) const
{
...
void CHTRichEditCtrl::AddString(int nPos, LPCTSTR pszString, bool bLink, COLORREF cr)
{
	...
	//else if (cr != CLR_DEFAULT)
	{
		...
		cf.crTextColor = GetLogLineColor(pszString, (_TUCHAR)pszString[0]);
		SetSelectionCharFormat(cf);
		bRestoreFormat = true;
	}
...
void CHTRichEditCtrl::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	menu.AppendMenu(MF_STRING | (lSelEnd > lSelStart ? MF_ENABLED : MF_GRAYED), MP_COPYSELECTED, IDS_COPY);
	menu.AppendMenu(MF_SEPARATOR);
	menu.AppendMenu(MF_STRING | (iTextLen > 0 ? MF_ENABLED : MF_GRAYED), MP_SELECTALL, (IDS_SELECTALL));
	menu.AppendMenu(MF_STRING | (iTextLen > 0 ? MF_ENABLED : MF_GRAYED), MP_REMOVEALL , (IDS_PW_RESET));
	menu.AppendMenu(MF_STRING | (iTextLen > 0 ? MF_ENABLED : MF_GRAYED), MP_SAVELOG, CString(IDS_SAVELOG) + _T("..."));
	menu.AppendMenu(MF_SEPARATOR);
	menu.AppendMenu(MF_STRING | (m_bAutoScroll ? MF_CHECKED : MF_UNCHECKED), MP_AUTOSCROLL, (IDS_AUTOSCROLL));
...
{
	bool bResult = false;
	CFileDialog dlg(FALSE, _T("log"), pszDefName ? pszDefName : (LPCTSTR)m_strTitle, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("Log Files (*.log)|*.log||"));
	if (dlg.DoModal() == IDOK)
	{
...
int CHTRichEditCtrl::GetTextRange(int nFirst, int nLast, CString& refString) const
{
	ASSERT(::IsWindow(m_hWnd));
...
void CHTRichEditCtrl::CreateYourself(CWnd* pWnd, HWND hwnd)
{
	CRect rc;
	::GetClientRect(hwnd, rc);
	::MapWindowPoints(NULL, pWnd->m_hWnd, (LPPOINT)&rc, 2);
	Create(WS_CHILD | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_HSCROLL | ES_AUTOHSCROLL   | WS_VSCROLL | ES_MULTILINE | ES_READONLY, rc, pWnd, (UINT)-1);
	ModifyStyleEx(0, WS_EX_STATICEDGE, SWP_FRAMECHANGED);
	SendMessage(EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN, MAKELONG(3, 3));
	SetEventMask(GetEventMask() | ENM_LINK);
	CFont fontHyperText;
	fontHyperText.CreatePointFont(10 * 8, _T("MS Shell Dlg"));
	SetFont(&fontHyperText);
	SetProfileSkinKey(_T(""));
	ApplySkin();
	//m_log.AppendKeyWord("LOG", RGB(255,0,0));
	SetTitle("title");
