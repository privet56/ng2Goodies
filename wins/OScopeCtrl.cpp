...
#define UM_OSCOPEPOSITION  (WM_USER + 0x100 + 2)
//extern "C" int __cdecl __ascii_stricmp(const char * dst, const char * src);
int __ascii_stricmp(CIPString dst, CIPString src)
{
	return dst.CompareNoCase(src);
}
...
void COScopeCtrl::InvalidateCtrl(bool deleteGraph)
{
	...
	*/
	...
	BOOL m_bShowVerticalHourMarkers = TRUE;
	if (m_bShowVerticalHourMarkers)
	{
...
void COScopeCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (pwndParent)
	{
		...
		int shownsecs = plotRect.Width() * 5;//thePrefs.GetTrafficOMeterInterval();
