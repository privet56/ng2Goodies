...
#include <Winperf.h>
#include <stdio.h>
#include <comutil.h>	// for using bstr_t class
//#include <comdef.h>	// for using bstr_t class
//#include "cpuscanner.h"
#include "ipstring.h"
#include "wins.h"
	...
	T GetCounterValue(PERF_DATA_BLOCK **pPerfData, DWORD dwObjectIndex, DWORD dwCounterIndex, LPCTSTR pInstanceName = NULL)
	{
		...
		for( DWORD i=0; i < (*pPerfData)->NumObjectTypes; i++ )
		{
			...
			if (pPerfObj->ObjectNameTitleIndex == dwObjectIndex)
			{
				...
				{
					__Log("GetCounterValue\tWRN:(1)app.nf:"+CIPString(pInstanceName));
				}
				break;
			}
		...
		if(!bFound)
		{
			__Log("GetCounterValue\tWRN:app.nf:"+CIPString(pInstanceName));
		}
		else if(bFound && (lnValue > -1))
		{
			//CIPString s;
			//s.Format("object:%d, value:%d", dwObjectIndex, lnValue);
			//__Log("GetCounterValue\tINF:app.found:"+s+"\t"+CIPString(pInstanceName));
		}
		else if(bFound && (lnValue < 0))
		{
			//not found but logged
		}
		else
		{
			__Log("GetCounterValue\tWRN:???:app.nf:"+CIPString(pInstanceName));
...
protected:
	...
	T GetCounterValue(PPERF_OBJECT_TYPE pPerfObj, DWORD dwCounterIndex, LPCTSTR pInstanceName)
	{
		...
		else
		{
			...
			BSTR bstrInstance;
			BSTR bstrInputInstance = (BSTR)pInstanceName;
			for( int k=0; k < pPerfObj->NumInstances; k++ )
			{
				bstrInstance = (wchar_t *)((PBYTE)pPerfInst + pPerfInst->NameOffset);
				//LONG luniqid = (LONG)((PBYTE)pPerfInst + pPerfInst->UniqueID);
				CIPString sui;
				//sui.Format("(id:%d)",luniqid);

				CIPString s1(bstrInstance);
				CIPString s2(pInstanceName);

				//if (!stricmp((LPCTSTR)bstrInstance, (LPCTSTR)pInstanceName))
				//__Log(s1+sui+"<--->"+s2);
		...
		if(!bFound)
		{
			__Log("GetCounterValue\tWRN:perfobj.nf:"+CIPString(pInstanceName));
		}
	...
	T GetCounterValueForProcessID(PPERF_OBJECT_TYPE pPerfObj, DWORD dwCounterIndex, DWORD dwProcessID)
	{
		...
		for( DWORD j=0; j < pPerfObj->NumCounters; j++ )
		{
			if (pPerfCntr->CounterNameTitleIndex == (DWORD)PROC_ID_COUNTER)
			{
